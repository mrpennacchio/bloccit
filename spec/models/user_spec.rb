...
RSpec.describe User, type: :model do
  ...
  describe "#favorite_for(post)" do
    ...
    it "returns 'nil' if the user has not favorited the post" do
      # expect that favorite_for will reutnr nil if the user has not favorited @post
  ...
  # use .avarar_url because the . says its a class method.
  describe ".avatar_url" do
    # build a user with FactoryGirl. pass an email in so that the test will returna known email gravatar
    let(:known_user) { create(:user, email: "blochead@bloc.io") }

    it "returns the proper Gravatar url for a known email entity" do
      # expected string gravatar should return. s=48 specifies that we want the image to be 48x48 px
